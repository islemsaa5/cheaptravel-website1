...
const BookingForm: React.FC<BookingFormProps> = ({
  ...
  const validateTravelers = () => {
    ...
    travelerDetails.forEach((t, idx) => {
      if (!t.firstName) errors.push(`Prénom manquant pour le voyageur ${idx + 1}`);
      if (!t.lastName) errors.push(`Nom manquant pour le voyageur ${idx + 1}`);
      if (!t.dateOfBirth) errors.push(`Date de naissance manquante pour le voyageur ${idx + 1}`);
      if (!t.passportNumber) errors.push(`Numéro de passeport manquant pour le voyageur ${idx + 1}`);
    });

    if (!contactData.email) errors.push("L'email de contact est obligatoire");
    if (!contactData.phone) errors.push("Le numéro de téléphone est obligatoire");
    ...
    <div className="bg-white rounded-[40px] shadow-2xl border border-gray-100 overflow-hidden max-w-4xl mx-auto animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="bg-blue-900 p-8 text-white flex flex-col md:flex-row items-center justify-between border-b border-white/10 sticky top-0 z-50 shadow-md">
        ...
        <div className="mt-4 md:mt-0 text-right">
          ...
          <p key={totalPrice} className="text-3xl font-black text-orange-500 animate-in zoom-in spin-in-3 duration-300">{totalPrice.toLocaleString()} DA</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-8 md:p-12">
        ...
        {formErrors.length > 0 && (
          <div className="mb-8 bg-red-50 border border-red-100 p-6 rounded-3xl animate-in fade-in slide-in-from-top-2">
            <div className="flex items-center space-x-3 text-red-600 mb-2">
              <AlertTriangle size={18} />
              <h4 className="font-black text-sm uppercase tracking-widest">Informations Manquantes</h4>
            </div>
            <ul className="list-disc list-inside space-y-1 ml-2">
              {formErrors.map((err, i) => (
                <li key={i} className="text-xs font-bold text-red-400">{err}</li>
              ))}
            </ul>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="space-y-10 animate-in fade-in slide-in-from-right-4">
            ...
            <button type="button" onClick={() => setStep(2)} className="w-full bg-blue-900 text-white font-black py-6 rounded-[28px] shadow-2xl shadow-blue-900/20 uppercase tracking-widest text-xs hover:bg-black transition-all">Continuer vers les Identités</button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
            ...
            <div className="flex gap-4">
              ...
              <button type="button" onClick={handleNextStep} className="flex-[2] bg-blue-900 text-white font-black py-5 rounded-2xl uppercase tracking-widest text-[10px]">Passer au Paiement</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
            ...
            {paymentMethod === 'CIB_EDAHABIA' && (
              <div className="bg-white p-10 rounded-[48px] border border-blue-100 shadow-xl flex flex-col items-center justify-center text-center space-y-8 animate-in slide-in-from-top-4 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-12 opacity-5 pointer-events-none">
                  <ShieldCheck size={200} className="text-blue-900" />
                </div>

                <div className="relative z-10 bg-blue-50 p-6 rounded-3xl border border-blue-100">
                  <div className="flex items-center space-x-6">
                    <div className="bg-white p-3 rounded-xl shadow-sm"><CreditCard className="text-blue-900" size={32} /></div>
                    <div className="h-8 w-px bg-blue-200"></div>
                    <div className="text-left">
                      <h5 className="font-black text-blue-900 text-sm">Passerelle de Paiement SATIM</h5>
                      <p className="text-[10px] uppercase font-bold text-blue-400 tracking-widest">Connexion Interbancaire Sécurisée</p>
                    </div>
                  </div>
                </div>

                <div className="max-w-md space-y-4">
                  <h4 className="text-2xl font-black text-blue-900 tracking-tight">Redirection Paiement Sécurisé</h4>
                  <p className="text-gray-500 font-medium leading-relaxed">
                    Pour garantir une sécurité maximale (PCI-DSS), le paiement par carte CIB ou Edahabia s'effectue directement sur le serveur de la banque.
                  </p>
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center space-x-3 text-green-600 justify-center bg-green-50 py-2 rounded-lg">
                      <Lock size={14} />
                      <span className="text-[10px] font-black uppercase tracking-widest">Chiffrement SSL 256-bit</span>
                    </div>
                    <div className="flex items-center space-x-3 text-blue-600 justify-center bg-blue-50 py-2 rounded-lg">
                      <Fingerprint size={14} />
                      <span className="text-[10px] font-black uppercase tracking-widest">Authentification 3D-Secure</span>
                    </div>
                  </div>
                </div>

                <div className="bg-orange-50 text-orange-600 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest animate-pulse border border-orange-100">
